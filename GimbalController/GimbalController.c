...
void main(void)
{
    ...
    while (1)
    {
        ...
        {
            DMPReady_flag = 0;
            if (Read_DMP(&RPY[0], &RPY[1], &RPY[2]) == 0)
            {

                if ((RPY[1] < 0) && (RPY[1] >= -180))
                {
                    RPY[1] += 180;
                }
                else
                ...
                pid_Pitch.ActualValue = RPY[1];
                theta_Pitch += PID_Increase_Pitch(90 + deviation_Pitch + origin_Pitch);
